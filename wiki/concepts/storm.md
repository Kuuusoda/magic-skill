...
- **只看该 storm 咒语之前**：响应 storm 触发式异能或响应原咒语而施放的后续咒语，不会被这个 storm 计数。
- **不要混淆其他条件检查**：如果原咒语结算时另有 “if you've cast another instant or sorcery spell this turn” 之类条件，那是该咒语自己的结算检查；除非文字写明“before it”，否则会看本回合到结算时为止的事实，而不是 storm 的“之前”计数。
