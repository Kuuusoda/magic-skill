...
| 组件 | 作用 |
|------|------|
| **塔萨的先知 / Thassa's Oracle** | ETB：检视牌库顶 X 张牌（X=对蓝献忠）；若 X ≥ 牌库牌数，赢得游戏 |
| **恶魔咨商 / Demonic Consultation** | {B}：说出牌名，先放逐顶 6 张再展示放逐直到找到该牌；说出库中无的牌名则整个牌库被放逐 |
| **腐化协定 / Tainted Pact** | {1}{B}：逐张放逐牌库顶并选择不留手，直到放逐两张同名牌为止；单卡构筑下可放逐整个牌库（无生命损失） |
