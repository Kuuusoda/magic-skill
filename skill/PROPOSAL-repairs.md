...
- 版本:v0.3(保留 v0.2 原提案正文,补 2026-07-09 实施状态)
...
本文件以下 R1-R6 正文保留原提案口径,用于追溯问题与 gate；当前状态如下:

- **R1 已实施**:根级 `SKILL.md` 已删除;有效 skill 入口以 `skill/<name>/SKILL.md` 为准。
- **R2 已实施方案 A**:运行时能力声明已改为“本地索引可选、缺失时 mtgch/Scryfall API 回退”;README、agent、`mtg-wiki` skill 与 Tameshi Belcher 页面已去掉“内置 37k 本地库/O(1)/不联网”类夸大。构建期历史统计仍保留。
- **R3 已实施主路径**:`skill/_shared/mtg-common.md` 已创建并通过 `opencode.json` 的 `instructions` 注入主会话;子 agent 继承性仍为待实测项。
- **R4/R5/R6 未在本轮实施**:宪法定稿、Modern 孤儿页/撞名数据修复、cEDH 概念层无源时效数据清理仍按原 gate 后续处理。
